...
class Graph:
    def __init__(self, G, pos,nodeLabel, elevation = None):
        self.G = G
        self.pos = pos
    ...
    def color(self):
        ...
        for i in range(len(self.nodeLabel)):
            d = self.elevation.get(self.nodeLabel[i])
            # self.color_map.append( (0 , 0, 1, 3*d/50 ) )
            
            
            self.color_map.append( ((1-sqrt(d/200)), 0.01, sqrt(d/200), 0.5) )
            """
            if self.pickup_intersection(self.nodeLabel[i]):
                # self.color_map.append( (0 , 0, 0.5, d**2/100 ) )
                print(sqrt(d/120))
                self.color_map.append( ((1-sqrt(d/200))/2, 0.01, sqrt(d/200), 0.5) )
            else:
